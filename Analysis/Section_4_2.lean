...
instance Rat.mul_inst : Mul Rat where
  mul := Quotient.lift₂ (fun ⟨ a, b, h1 ⟩ ⟨ c, d, h2 ⟩ ↦ (a*c) // (b*d)) (by
    ...
    simp_all [Rat.eq, PreRat.eq]
    suffices h: a1n * a2d * (b1n * b2d) = a2n * a1d * (b2n * b1d) from by
      linarith
    repeat simp_all)
...
instance Rat.instField : Field Rat where
  ...
  mul_inv_cancel := by
    ...
    rw [x4, inv_eq, mul_eq, of_Nat_eq, eq]
    simp_all
    ring_nf
    by_cases h1 : x1 ≠ 0
    . simp_all
    push_neg at h1
    rw [h1] at x4
    simp [x4, of_Nat_eq] at h
    rw [eq] at h
    rw [Int.zero_mul, zero_mul] at h
    contradiction
